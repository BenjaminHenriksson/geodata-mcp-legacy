...
So `render_map` exists. Give it a list of layers; it returns a PNG
URL valid for 24 hours. The model takes that URL and embeds the image
in whatever document it composes. No PDF-layout tool needed on our
side, regardless of which client (claude.ai, ChatGPT, Gemini, etc.)
is driving.
...
The same spec `layer(op="show")` accepts is the one `render_map` reads:
